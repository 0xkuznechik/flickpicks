...
const keyDates = [
  ...
  { date: "Jan 27", event: "British Academy Film Awards (BAFTA) Nominations Announcement" },
  { date: "Jan 27", event: "American Cinema Editors (ACE) Nominations Announcement" },
  { date: "Jan 27", event: "Writers Guild of America (WGA) Nominations Announcement" },
  ...
  { date: "Feb 14", event: "Make-Up Artists & Hair Stylists Guild (MUAHS) Awards Ceremony" },
  ...
  { date: "Feb 22", event: "British Academy of Film and Television Arts (BAFTA) Awards Ceremony" },
  ...
  { date: "March 1", event: "Actor Awards presented by SAG-AFTRA Awards Ceremony" },
  { date: "March 5", event: "Finals Voting Ends", highlight: true },
  { date: "March 8", event: "Writers Guild of America (WGA) Awards Ceremony" },
  { date: "March 8", event: "American Society of Cinematographers (ASC) Awards Ceremony" },
  { date: "March 13", event: "LAST DAY TO LOCK IN BETS", highlight: true, color: "text-gold-400" },
    ...
    <div className="min-h-screen bg-black text-zinc-100 font-sans selection:bg-gold-500/30">
      ...
      <nav className="border-b border-white/10 bg-black py-4">
        <div className="container-pad flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <img src="/images/oscars-statuettes.png" alt="Logo" className="h-10 w-auto" /> {/* Assuming generic logo or statuette */}
            <span className="font-[var(--font-cinzel)] text-xl tracking-widest text-gold-400">PICKS</span>
          </div>
          <div className="flex-1 flex justify-end gap-4">
            {!user && (
              <>
                <Link to="/login" className="text-xs font-semibold uppercase tracking-wider text-zinc-300 hover:text-white border border-zinc-700 px-3 py-1 rounded">Log In</Link>
                <Link to="/join" className="text-xs font-semibold uppercase tracking-wider text-black bg-gold-400 hover:bg-gold-500 px-3 py-1 rounded">Sign Up</Link>
              </>
            )}
            {user && (
        ...
        <div className="mt-4 flex justify-center gap-8 border-t border-white/10 py-3 text-sm font-medium tracking-wide text-zinc-400">
          <Link to="/" className="text-gold-400 hover:text-gold-300">Home</Link>
          <Link to="/ballot" className="hover:text-zinc-200">My Bets</Link>
        </div>
      </nav>
      ...
      <main className="container-pad space-y-20 py-12">
        ...
        <div className="flex items-center gap-4">
          ...
          <Link to={ctaHref} className="rounded bg-gold-400 px-10 py-3 text-lg font-bold text-black shadow-[0_0_20px_rgba(231,200,106,0.3)] hover:bg-gold-500 hover:shadow-[0_0_30px_rgba(231,200,106,0.5)] transition-all">
            Bet Now!
          </Link>
          <div className="h-px flex-1 bg-gradient-to-r from-transparent via-zinc-700 to-transparent"></div>
        </div>
        ...
        <section className="rounded-2xl border border-white/10 bg-zinc-900/30 p-8">
          <h2 className="mb-8 text-center font-[var(--font-cinzel)] text-2xl font-bold text-white">Key Event Dates</h2>
          <div className="mx-auto max-w-3xl space-y-3">
            {keyDates.map((item, idx) => (
              <div key={idx} className="flex gap-3 text-sm">
                <span className={`w-20 flex-shrink-0 font-bold ${item.color || "text-gold-400"}`}>{item.date} —</span>
                <span className={item.highlight ? "font-semibold text-white" : "text-zinc-400"}>
                  {item.event}
                </span>
              </div>
            ))}
